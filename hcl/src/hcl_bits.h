...
class bits_t
{
    ...
    public:
        iterator(uint64_t _value, bool begin) : value_(_value), position_(begin ? START_ITR : END_ITR) { if (begin) next_bit(0); }
        iterator& operator++() { next_bit(); return *this; }
    ...
    public:
        ...
        bit_ref& operator = (bool _x) { bits_.set(pos_, _x); return *this; }
...
protected:
    ...
    #define _INIT_ raw_ = 0; for (auto b : bits) set(b);
...
public:
    ...
    bits_t& operator = (uint64_t  val)  { raw_ = val; return *this; }
    bits_t& operator = (init_list bits) { init(bits); return *this; }
    bits_t& operator = (uint_set  bits) { init(bits); return *this; }
