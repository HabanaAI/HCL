...
protected:
    ...
    const scal_stream_info_t& m_streamInfo;    // Scal's stream info for commands alignment and submission alignment
    const uint64_t            m_divAlignment;  // SW alignment for managing the cyclic buffer
    unsigned                  m_divIndex;      // Current division we are on
    std::array<uint64_t, m_numberOfDivisions>
                                          m_targetValueOfBufferChunk;  // the target value of the job of each division
    std::array<bool, m_numberOfDivisions> m_targetValueOfBufferSet;
