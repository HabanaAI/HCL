...
public:
    ...
    virtual void addNicBuffer(const NicsDwordsArray& nicBuffer);  // for nic macro pair
    int
    addDeviceBuffer(const DwordsBoxesArray& deviceBuffer,
                    const DevicesSet&       devicesSet);  // Add all devices vectors of dwords to internal structures,
    ...
    int flush(hcl::ScalStreamBase& scalStream,
              const uint16_t       setNopDupMask);  // returns number of dwords used after aggregation
