...
public:
    ...
    nics_mask_t getEnabledPortsMask() const { return m_enabled_ports_mask; }              // After mask by LKD only
    uint16_t    getDefaultScaleUpPort(const HCL_Comm hclCommId = DEFAULT_COMM_ID) const;  // No masks
    uint64_t    getExternalPortsMask(const HCL_Comm hclCommId = DEFAULT_COMM_ID) const;   // Includes LKD & HCL Masks
    nics_mask_t
                     getAllPorts(const int      deviceId,
    ...
    uint16_t         getNumScaleOutPorts(const HCL_Comm hclCommId = DEFAULT_COMM_ID) const;  // Includes LKD & HCL Masks
    nics_mask_t      getScaleOutPorts(const HCL_Comm hclCommId = DEFAULT_COMM_ID) const;     // Includes LKD & HCL Masks
    nics_mask_t      getScaleUpPorts(const HCL_Comm hclCommId = DEFAULT_COMM_ID) const;
    uint16_t         getScaleoutSubPortIndex(const uint16_t port,
    ...
    uint16_t         getMaxNumScaleOutPorts() const { return m_max_scaleout_ports; };  // Includes LKD mask only
    ...
    nics_mask_t      getLkdEnabledScaleoutPorts() const { return m_lkd_enabled_scaleout_ports; };
...
protected:
    ...
    nics_mask_t m_enabled_ports_mask = INVALID_PORTS_MASK;  // After mask by LKD only, includes scaleup
    nics_mask_t m_lkd_enabled_scaleout_ports;               // After masking by LKD only
    uint16_t    m_max_scaleout_ports;                       // After masking by LKD only
