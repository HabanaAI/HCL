...
#ifndef ASIC_REG_STRUCTS_GAUDI3_SPECIAL_REGS_H_
#define ASIC_REG_STRUCTS_GAUDI3_SPECIAL_REGS_H_
...
/*
 MEM_GW_DATA 
 b'Write Data on Write Read Data on Read'
*/
...
/*
 MEM_GW_REQ 
 b'Command and Address for MEM_GW access'
*/
...
/*
 MEM_ECC_ERR_CNT 
 b'SERR/DERR counter, Threshold indication'
*/
typedef struct reg_mem_ecc_err_cnt {
	union {
		struct {
			uint32_t cnt_serr : 8,
				serr_th_hit : 1,
...
static_assert((sizeof(struct reg_mem_ecc_err_cnt) == 4), "reg_mem_ecc_err_cnt size is not 32-bit");
/*
 MEM_RM 
...
struct block_special_regs {
	...
	struct special_regs::reg_mem_ecc_err_cnt mem_ecc_err_cnt;
...
typedef struct block_special_regs {
	...
	reg_mem_ecc_err_cnt mem_ecc_err_cnt;
	reg_mem_rm mem_rm;
	uint32_t _pad176[4];
...
#pragma pack(pop)
#endif /* ASIC_REG_STRUCTS_GAUDI3_SPECIAL_REGS_H_ */
